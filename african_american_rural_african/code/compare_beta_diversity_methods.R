...
pcoa_function <- function(dist_df = bc , meta_df = meta, group = "SampleType", distance_metric = "Bray Curtis Dissimilarity"){
  ...
  return(list(plot_beta,pco_df))
  
}



nmds_function <- function(dist_df = bc , meta_df = meta, group = "SampleType", distance_metric = "Bray Curtis Dissimilarity"){
  
  # nonmetric multidimensional scaling
  pco <- vegan::metaMDS(dist_df) 
...
pcoa_bc_df <- pcoa_bc[[2]][,1:2]
nmds_bc_df <- nmds_bc[[2]][,1:2]
...
pcoa_aitch_df <- pcoa_aitch[[2]][,1:2]
nmds_aitch_df <- nmds_aitch[[2]][,1:2]
...
pcoa_unifrac_df <- pcoa_unifrac[[2]][,1:2]
nmds_unifrac_df <- nmds_unifrac[[2]][,1:2]
...
pcoa_rao_df <- pcoa_rao[[2]][,1:2]
nmds_rao_df <- nmds_rao[[2]][,1:2]
...
# Compare distances
calculate_stress(as.matrix(dist_btw),as.matrix(w_unifrac))
calculate_stress(as.matrix(dist_btw),as.matrix(bc))
calculate_stress(as.matrix(dist_btw),as.matrix(aitch))


# Save plots
pdf(file.path(output_dir,"pcoa_nmds_plots.pdf"),20,20)
